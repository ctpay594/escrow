...
interface CompanyBulkTransferPanelProps {
  availableBalance: number;
  disabled?: boolean;
  onSubmitted?: () => void;
}

export function CompanyBulkTransferPanel({
  availableBalance,
  disabled = false,
  onSubmitted,
}: CompanyBulkTransferPanelProps) {
  ...
  async function submitBulkTransfer() {
    ...
    try {
      const response = await fetch('/api/transfers/company/bulk', {
        method: 'POST',
      ...
      toast.success(
        `${data.transfer_count} company transfers submitted`,
      );
      ...
      <GlassCard>
        <GlassCardHeader className="pb-3">
          <GlassCardTitle className="text-base">
            Bulk from company account
          </GlassCardTitle>
          <GlassCardDescription>
            Upload Excel or CSV. Sends from the HDFC current account — not from
            any merchant balance. Columns can be in any order.
          </GlassCardDescription>
        </GlassCardHeader>
