...
export function CompanyTransferWizard({
  availableBalance,
  disabled = false,
  onSubmitted,
}: {
  ...
  async function submitTransfer() {
    ...
    try {
      const response = await fetch('/api/transfers/company', {
        method: 'POST',
      ...
      setSubmitted(true);
      onSubmitted?.();
      toast.success('Company transfer submitted');
    } catch (err) {
      toast.error(
    ...
    <>
      <GlassCard className={disabled ? 'opacity-60' : undefined}>
        <GlassCardHeader className="p-5 pb-3">
          <GlassCardTitle>
            {submitted ? 'Transfer submitted' : 'Route from company account'}
          </GlassCardTitle>
          <GlassCardDescription>
            {submitted
              ? 'Payout left the HDFC current account and is processing.'
              : `Company available ${formatCurrency(availableBalance)} · ${payoutMode}`}
          </GlassCardDescription>
        </GlassCardHeader>
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This sends money from the HDFC current account. It does not debit
              any merchant ledger.
            </DialogDescription>
          </DialogHeader>
